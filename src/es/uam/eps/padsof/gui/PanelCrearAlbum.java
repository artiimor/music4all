...
public class PanelCrearAlbum extends JPanel implements ActionListener, MouseListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private JButton boton = new JButton("Siguiente");
	private JLabel etiquetaAlbum = new JLabel("Titulo");
	private JTextField titulo = new JTextField(19);
	private JLabel etiquetaElementos = new JLabel("Elementos del Album");
	private JList<String> canciones;
	private Integer[] ids;
	private JScrollPane scroll;
	private Gui gui;

	PanelCrearAlbum(Gui gui, String[] can, Integer[] ids) {
		this.gui = gui;
		this.ids = ids;
		...
		this.add(etiquetaAlbum);
		this.add(titulo);
		this.add(etiquetaElementos);
		this.add(scroll);
		this.add(boton);

		layout.putConstraint(SpringLayout.WEST, etiquetaAlbum, 45, SpringLayout.WEST, this);
		layout.putConstraint(SpringLayout.NORTH, etiquetaAlbum, 20, SpringLayout.NORTH, this);
		layout.putConstraint(SpringLayout.WEST, titulo, 45, SpringLayout.WEST, this);
		layout.putConstraint(SpringLayout.NORTH, titulo, 5, SpringLayout.SOUTH, etiquetaAlbum);
	...
	public void actionPerformed(ActionEvent e) {
		gui.getControlador().controladorSubirAlbum(titulo.getText(),ids);
	}
